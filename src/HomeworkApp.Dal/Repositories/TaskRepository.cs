...
";
    ...
    public async Task<SubTaskEntityV1[]> GetSubTasksInStatus(long parentTaskId, IEnumerable<int> statuses, CancellationToken token)
    {
...
with recursive tasks_tree
       as (select t.id,
                  ...
                  t.title,
                  t.status,
                  array[t.parent_task_id] as parents_ids
           from tasks t
           ...
           select t.id,
                  ...
                  t.title,
                  t.status,
                  parents_ids || t.parent_task_id as parents_ids
           from tasks t
...
";
        ...
        if (statuses.Any())
        {
            ...
            @params.Add($"Statuses", new int[]{1, 2, 3, 4, 5});
        }
        ...
        await using var connection = await GetConnection();
        return (await connection.QueryAsync<SubTaskEntityV1>(cmd)).ToArray();
