...
public class TaskService : ITaskService
{
    ...
    public async Task<GetTaskModel?> GetTask(
        ...
        CancellationToken token)
    {
        var cacheKey = $"cache_task:{taskId}";
    ...
    public async Task<TaskMessage[]?> GetComments(long taskId, CancellationToken token)
    {
        ...
        for (var i = 0; i < 5 && i < task.Length; i++)
        {
            result.Add(new TaskMessage
            {
                TaskId = task[i].TaskId,
                Comment = task[i].Message,
                IsDeleted = false,
                At = task[i].At,
            });
        }

        var taskJson = JsonSerializer.Serialize(result.ToArray());
        await _distributedCache.SetStringAsync(
            cacheKey, 
            taskJson,
            new DistributedCacheEntryOptions()
